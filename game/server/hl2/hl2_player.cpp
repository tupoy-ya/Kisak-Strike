...
bool CHL2_Player::ApplyBattery( float powerMultiplier )
{
	...
	if ((ArmorValue() < MAX_NORMAL_BATTERY) && IsSuitEquipped())
	{
		...
		CHLUsrMsg_ItemPickup msg;
		msg.set_item( "item_battery" );
		SendUserMessage( user, HL_UM_ItemPickup, msg );
...
void CHL2_Player::OnSquadMemberKilled( inputdata_t &data )
{
	...
	CHLUsrMsg_SquadMemberDied msg;
	SendUserMessage( user, HL_UM_SquadMemberDied, msg );
}
...
int CHL2_Player::GiveAmmo( int nCount, int nAmmoIndex, bool bSuppressSound)
{
	...
	if ( nCount > 0 && nAdd == 0 )
	{
		...
		CHLUsrMsg_AmmoDenied msg;
		msg.set_ammoidx( nAmmoIndex );
		SendUserMessage( user, HL_UM_AmmoDenied, msg );
	}
...
void CHL2_Player::UpdateClientData( void )
{
	if (m_DmgTake || m_DmgSave || m_bitsHUDDamage != m_bitsDamageType)
	{
		...
		CHLUsrMsg_Damage msg;

		msg.set_amount( (int )m_DmgTake );
		msg.set_victim_entindex( entindex() );

		msg.mutable_inflictor_world_pos()->set_x( damageOrigin.x );
		msg.mutable_inflictor_world_pos()->set_y( damageOrigin.y );
		msg.mutable_inflictor_world_pos()->set_z( damageOrigin.z );

		SendUserMessage( user, HL_UM_Damage, msg );
